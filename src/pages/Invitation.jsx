...
function Invitation() {
  ...
  useEffect(() => {
    if (slug) {
      axios
        .get(`http://localhost:3000/invitations?slug=${slug}`)
        .then((response) => {
  ...
  return (
    <div className="font-serif bg-gray-100 min-h-screen p-8">
      <div className="max-w-4xl mx-auto bg-white shadow-2x1 rounded-lg p-12">
        <header className="text-center border-b-2 pb-8 border-gray-200">
          <h1 className="text-5xl font-bold text-gray-800">
            {details.namaMempelaiPria} & {details.namaMempelaiWanita}
          </h1>
          <p className="text-xl text-gray-500 mt-4">
            Mengundang Anda ke pernikahan kami
          </p>
        </header>
        ...
        <section className="mt-10 text-center">
          <p className="text-lg text-gray-600">
            ...
            hadir di acara pernikahan kami yang akan diselenggarakan pada:
          </p>
          <div className="mt-8">
            <p className="text-2xl font-semibold text-gray-700">
              {new Date(details.tanggalAcara).toLocaleDateString("id-ID", {
                weekday: "long",
                year: "numeric",
                month: "long",
                day: "numeric",
        ...
        <section className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div className="text-center">
            <img
              src={details.fotoMempelaiPria}
              alt={details.namaMempelaiPria}
              className="w-48 h-48 rounded-full object-cover mx-auto shadow-lg"
            />
            <h3 className="text-2xl font-bold mt-4 text-gray-800">
              {details.namaMempelaiPria}
            </h3>
            <p className="text-gray-600">
              Putra dari Bpk. {details.namaAyahMempelaiPria} & Ibu{" "}
              {details.namaIbuMempelaiPria}
            </p>
          </div>
          <div className="text-center">
            <img
              src={details.fotoMempelaiWanita}
              alt={details.namaMempelaiWanita}
              className="w-48 h-48 rounded-full object-cover mx-auto shadow-lg"
            />
            <h3 className="text-2xl font-bold mt-4 texxt-gray-800">
              {details.namaMempelaiWanita}
            </h3>
            <p className="text-gray-600">
              Putri dari Bpk. {details.namaAyahMempelaiWanita} & Ibu{" "}
              {details.namaIbuMempelaiWanita}
            </p>
          </div>
        </section>
