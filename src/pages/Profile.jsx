...
function Profile() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setInvitations(
        invitations.filter((inv) => !idsToDelete.includes(inv.id))
      );
    ...
    const fetchInvitations = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/invitations?user_id=${session.user.id}`
        );
  ...
  if (!session) {
    return (
      <div className="container mx-auto px-6 py-12 text-center dark:text-gray-300">
      ...
      <div className="bg-gray-50 dark:bg-gray-900 min-h-screen">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-100 dark:border-gray-700 mb-12">
            <div className="flex flex-col sm:flex-row items-center gap-8">
              ...
              <button
                ...
                className="bg-red-50 hover:bg-red-100 text-red-600 font-bold py-2 px-5 rounded-lg transition duration-300 text-sm flex items-center gap-2 self-center sm:self-start mt-4 sm:mt-0 border border-red-200 dark:bg-red-900 dark:hover:bg-red-800 dark:text-red-300 dark:border-red-700"
              >
          ...
          <div>
              ...
              {invitations.length > 0 && (
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="bg-red-500 text-white font-bold py-2 px-4 rounded-lg disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center gap-2 transition-colors"
                  >
            ...
            {error && (
              <p className="text-center text-red-500 bg-red-50 p-4 rounded-lg dark:text-red-300 dark:bg-red-900">
                Error: {error}
              </p>
            )}
            ...
            {!loading && !error && invitations.length === 0 && (
              <div className="text-center bg-white dark:bg-gray-800 rounded-lg p-12 shadow-md">
                ...
                <p className="text-gray-600 dark:text-gray-300 mt-4 text-lg">
                  Anda belum pernah membuat undangan.
                </p>
                <Link
                  to="/template"
                  className="mt-6 inline-block bg-gradient-to-r from-indigo-500 to-purple-600 text-white hover:from-purple-600 hover:to-indigo-500 transition-all duration-300 rounded-lg px-8 py-3 font-bold shadow-lg hover:shadow-xl transition-shadow"
                >
            ...
            {!loading && invitations.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {invitations.map((inv) => {
                  const title =
                    inv.generated_content.namaMempelaiPria &&
                    inv.generated_content.namaMempelaiWanita
                      ? `${inv.generated_content.namaMempelaiPria} & ${inv.generated_content.namaMempelaiWanita}`
                      : inv.slug || "Undangan";
                  const isSelected = selected.includes(inv.id);
                  return (
                    <div
                      key={inv.id}
                      className={`rounded-2xl shadow-lg border overflow-hidden flex flex-col group transition-all duration-300 bg-white dark:bg-gray-800 ${
                        isSelected
                    ...
                    >
                      <div className="p-6 flex-grow">
                        <div className="flex justify-between items-start">
                          <p className="text-sm text-gray-400 dark:text-gray-500 group-hover:text-blue-500 transition-colors">
                            {new Date(inv.id).toLocaleDateString("id-ID", {
                              year: "numeric",
                              month: "long",
                              day: "numeric",
                            })}
                          ...
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={() => handleSelection(inv.id)}
                            className="h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                          />
                        </div>
                        <h3
                          className="text-xl font-bold text-gray-800 dark:text-white mt-2 truncate"
                          title={title}
                        >
                          {title}
                        </h3>
                      ...
                      <div className="bg-gray-50 dark:bg-gray-700 p-4 border-t dark:border-gray-600 mt-auto flex items-center gap-2">
                        <a
                          ...
                          className="flex-1 text-center block bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-3 rounded-lg transition duration-300 text-sm"
                        >
                          Lihat Undangan
                        </a>
                        <button
                          onClick={() => handleDeleteRequest([inv.id])}
                          className="p-2 bg-red-100 hover:bg-red-200 text-red-600 rounded-lg transition duration-300 dark:bg-red-800 dark:hover:bg-red-700 dark:text-red-300"
            ...
            <div className="text-center">
              ...
              <div className="mt-4">
                <p className="text-base text-gray-600 dark:text-gray-300">
                  Apakah Anda yakin ingin menghapus {idsToDelete.length}{" "}
                  undangan? Tindakan ini tidak dapat dibatalkan.
                </p>
              </div>
            </div>
